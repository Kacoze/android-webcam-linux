...
case "${1:-}" in
    ...
    *)       echo "Usage: $0 {status|config|setup|logs}" >&2; exit 1 ;;
esac
...
cmd_start() {
    ...
    if ! validate_ip "$PHONE_IP"; then
        ...
        echo "Next: run '$0 config' and fix PHONE_IP" >&2
        return 11
    fi
    ...
    if [ -z "$SCRCPY_BIN" ]; then
        ...
        echo "Next: install scrcpy >= 2.0" >&2
        return 14
    fi
    ...
    if ! check_scrcpy_server_payload "$SCRCPY_BIN"; then
        ...
        echo "Next: reinstall scrcpy (Snap/Flatpak/system)" >&2
        return 16
    fi
    ...
    if ! sink_exists "$V4L2_SINK"; then
        if [ "$V4L2_SINK" != "/dev/video10" ]; then
            ...
            echo "Next: set V4L2_SINK in config or env" >&2
            return 13
        fi
        ...
        if ! command -v sudo >/dev/null 2>&1; then
            ...
            notify "critical" "Android Camera" "Video device not found" "error"
            echo "Next: install sudo and run 'sudo modprobe v4l2loopback'" >&2
            return 13
        fi
...
Or reboot your system." 2>/dev/null || true
    ...
    if [ ! -z "$EXTRA_ARGS" ]; then
        ...
        if [[ "$EXTRA_ARGS" =~ [\;\|\&\`\$\(\)\<\>] ]]; then
            ...
            echo "Next: remove unsafe chars from EXTRA_ARGS" >&2
            return 15
        fi
        ...
        for arg in "${EXTRA_ARGS_ARRAY[@]}"; do
            ...
            if [[ "$arg" =~ [\;\|\&\`\$] ]]; then
                ...
                echo "Next: remove unsafe args from EXTRA_ARGS" >&2
                return 15
            fi
...
cmd_logs() {
    ...
    case "${1:-}" in
        ...
        --sessions)
            if command -v ls >/dev/null 2>&1; then
                ls -1 "$LOG_DIR"/session-*.log 2>/dev/null || true
            fi
            return 0
            ;;
        --path)
            ...
            ;;
        *)
            echo "Usage: $0 logs [--path] [--sessions] [--follow|-f] [--tail N|-n N]" >&2
            return 1
            ;;
    esac
...
cmd_doctor() {
    ...
    if [ "$json_mode" = true ]; then
        ...
        printf '{"schema_version":1,"result":"%s","fails":%s,"warnings":%s,"checks":{%s},"suggested_actions":[%s]}\n' "$result" "$fails" "$warns" "$doctor_items" "$actions_json"
    else
        echo ""
...
case "$1" in
    ...
    *)
        ...
        echo "       $0 logs [--path] [--sessions] [--follow|-f] [--tail N|-n N]"
        exit 1
        ;;
esac
EOF
